...
public class Main {
	...
	private void showAllOrderItems() {
		...
		List<String> res = orders.stream()
									.flatMap(or -> orderToItemStringStream(or))
									.collect(Collectors.toList());
		res.forEach(System.out::println);
	}
	
	Stream<String> orderToItemStringStream(Order or){
		List<String> info = new ArrayList<String>();
		for(OrderItem oi: or.getOrderItems()) {
			info.add(oi.toString()); 
		}
		return info.stream();
	}
	
	static Stream<String> orderToStringStream(Order or) {
		List<String> strList = new ArrayList<String>();
		for(OrderItem i: or.getOrderItems())
